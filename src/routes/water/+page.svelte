<script>
  import ChemistryQuiz from "../../components/ChemistryQuiz.svelte";
  ...
  const pollutionQuestions = [
    {
      ...
      options: ["Plastique", "Carburant", "Gaz à effet de serre"],
      answer: "Plastique",
    },
    {
      question:
        "Quelle substance chimique issue des engrais peut polluer l'eau ?",
      options: ["Sel", "Nitrates", "Oxygène"],
      answer: "Nitrates",
    },
    {
      question: "Quel type de déchets est le plus fréquent dans les rivières ?",
      options: ["Algues", "Métaux lourds", "Plastique"],
      answer: "Plastique",
    },
    {
      question: "Quelle action contribue à réduire la pollution de l'eau ?",
      options: [
        "Recycler les déchets",
        "Jeter dans les rivières",
        "Brûler le plastique",
      ],
      answer: "Recycler les déchets",
    },
    {
      question: "Quel organisme peut être affecté par les polluants de l'eau ?",
      options: ["Poissons", "Mammifères", "Oiseaux"],
      answer: "Poissons",
    },
  ];
  ...
  const checkPH = () => {
    ...
    if (
      ...
      alert(
        "Valeurs invalides pour chaque flacon. Respectez les intervalles !"
      );
      return;
    }
    ...
    if (avg >= 6.8 && avg <= 7.2) {
      phSolved = true;
      alert(
        `✅ pH correct ! Moyenne : ${avg.toFixed(2)} → lettre obtenue : ${phLetter}`
      );
    } else {
  ...
  const checkCode = () => {
    ...
    if (
      ...
      phSolved &&
      qcmSolved &&
      codeInput.toUpperCase() === finalCode
    ) {
...
<div class="water-room">
  ...
  <div class="story-card-Waterroom">
    <p>
      🌊 La rivière principale de GaiaCorp est gravement polluée par des
      substances chimiques et des déchets. Votre mission : purifier l’eau en
      réussissant les épreuves de chimie, de pH et de connaissance
      environnementale. Sauvez la vie aquatique avant qu’il ne soit trop tard !
    </p>
  </div>
  ...
  {#if !currentGame}
    <div class="choice-menu">
      ...
      <div class="choice-buttons">
        <button on:click={() => (currentGame = "quiz")} disabled={quizSolved}
          >🔬 Équation chimique</button
        >
        <button on:click={() => (currentGame = "ph")} disabled={phSolved}
          >⚗️ Ajuster le pH</button
        >
        <button on:click={() => (currentGame = "qcm")} disabled={qcmSolved}
          >🌍 Quiz pollution</button
        >
    ...
    <div class="ph-section">
      ...
      {#each Object.keys(phValues) as s, i}
        <div>
          <label>
            Flacon {i + 1}
            {s === "s1" ? "(0-5)" : s === "s2" ? "(5-9)" : "(9-12)"} :
          </label>
          <input
            type="number"
            step="0.01"
            name={s}
            bind:value={phValues[s]}
            on:input={handlePHChange}
          />
        </div>
      {/each}
      ...
      {#if !qcmSolved && qcmQueue.length > 0}
        <div>
          ...
          <button
            class="check-btn"
            on:click={checkQcm}
            disabled={!selectedAnswer}>Valider</button
          >
        </div>
      {:else if qcmSolved}
...
<style>
  ...
  .menu-btn:hover:not(:disabled) {
    ...
    box-shadow: 0 10px 24px rgba(0, 140, 200, 0.22);
  }
